...
create table ACT_RU_EXECUTION (
	...
	TENANT_ID_ varchar(255) default '',
	NAME_ varchar(255),
	LOCK_TIME_  datetime year to fraction(5),
    primary key (ID_)
);
...
create table ACT_EVT_LOG (
    ...
    EXECUTION_ID_ varchar(64),
    TASK_ID_ varchar(64),
    TIME_STAMP_ datetime year to fraction(5) not null,
    USER_ID_ varchar(255),
    DATA_ BLOB,
    LOCK_OWNER_ varchar(255),
    LOCK_TIME_ datetime year to fraction(5),
...
alter table ACT_GE_BYTEARRAY
    ...
    references ACT_RE_DEPLOYMENT (ID_) constraint ACT_FK_BYTEARR_DEPL;
-- Total length of columns in constraint is too long.
alter table ACT_RE_PROCDEF
    add constraint
    unique (KEY_,VERSION_, TENANT_ID_) constraint ACT_UNIQ_PROCDEF;
    
alter table ACT_RU_EXECUTION
    add constraint
    foreign key (PROC_INST_ID_)
    references ACT_RU_EXECUTION (ID_) constraint ACT_FK_EXE_PROCINST ;

alter table ACT_RU_EXECUTION
    add constraint
    foreign key (PARENT_ID_)
    references ACT_RU_EXECUTION (ID_) constraint ACT_FK_EXE_PARENT;

alter table ACT_RU_EXECUTION
    add constraint
    foreign key (SUPER_EXEC_)
    references ACT_RU_EXECUTION (ID_) constraint ACT_FK_EXE_SUPER;

alter table ACT_RU_EXECUTION
    add constraint
    foreign key (PROC_DEF_ID_)
    references ACT_RE_PROCDEF (ID_) constraint ACT_FK_EXE_PROCDEF;

alter table ACT_RU_IDENTITYLINK
    add constraint
    foreign key (TASK_ID_)
    references ACT_RU_TASK (ID_) constraint ACT_FK_TSKASS_TASK ;

alter table ACT_RU_IDENTITYLINK
    add constraint
    foreign key (PROC_DEF_ID_)
    references ACT_RE_PROCDEF (ID_) constraint ACT_FK_ATHRZ_PROCEDEF ;

alter table ACT_RU_IDENTITYLINK
    add constraint
    foreign key (PROC_INST_ID_)
    references ACT_RU_EXECUTION (ID_) constraint ACT_FK_IDL_PROCINST;

alter table ACT_RU_TASK
    add constraint
    foreign key (EXECUTION_ID_)
    references ACT_RU_EXECUTION (ID_) constraint ACT_FK_TASK_EXE;

alter table ACT_RU_TASK
    add constraint
    foreign key (PROC_INST_ID_)
    references ACT_RU_EXECUTION (ID_) constraint ACT_FK_TASK_PROCINST;

alter table ACT_RU_TASK
  	add constraint
  	foreign key (PROC_DEF_ID_)
  	references ACT_RE_PROCDEF (ID_) constraint ACT_FK_TASK_PROCDEF;

alter table ACT_RU_VARIABLE
    add constraint
    foreign key (EXECUTION_ID_)
    references ACT_RU_EXECUTION (ID_) constraint ACT_FK_VAR_EXE;

alter table ACT_RU_VARIABLE
    add constraint
    foreign key (PROC_INST_ID_)
    references ACT_RU_EXECUTION(ID_) constraint ACT_FK_VAR_PROCINST;

alter table ACT_RU_VARIABLE
    add constraint
    foreign key (BYTEARRAY_ID_)
    references ACT_GE_BYTEARRAY (ID_) constraint ACT_FK_VAR_BYTEARRAY;

alter table ACT_RU_JOB
    add constraint
    foreign key (EXCEPTION_STACK_ID_)
    references ACT_GE_BYTEARRAY (ID_) constraint ACT_FK_JOB_EXCEPTION ;

alter table ACT_RU_EVENT_SUBSCR
    add constraint
    foreign key (EXECUTION_ID_)
    references ACT_RU_EXECUTION(ID_) constraint ACT_FK_EVENT_EXEC;

alter table ACT_RE_MODEL
    add constraint
    foreign key (EDITOR_SOURCE_VALUE_ID_)
    references ACT_GE_BYTEARRAY (ID_) constraint ACT_FK_MODEL_SOURCE ;

alter table ACT_RE_MODEL
    add constraint
    foreign key (EDITOR_SOURCE_EXTRA_VALUE_ID_)
    references ACT_GE_BYTEARRAY (ID_) constraint ACT_FK_MODEL_SOURCE_EXTRA ;

alter table ACT_RE_MODEL
    add constraint
    foreign key (DEPLOYMENT_ID_)
    references ACT_RE_DEPLOYMENT (ID_) constraint ACT_FK_MODEL_DEPLOYMENT ;

alter table ACT_PROCDEF_INFO
    add constraint
    foreign key (INFO_JSON_ID_)
    references ACT_GE_BYTEARRAY (ID_) constraint ACT_FK_INFO_JSON_BA ;

alter table ACT_PROCDEF_INFO
    add constraint
    foreign key (PROC_DEF_ID_)
    references ACT_RE_PROCDEF (ID_) constraint ACT_FK_INFO_PROCDEF ;

alter table ACT_PROCDEF_INFO
    add constraint
    unique (PROC_DEF_ID_) constraint ACT_UNIQ_INFO_PROCDEF;